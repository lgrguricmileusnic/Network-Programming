...
int selectw(int nfds, fd_set *readfds, fd_set *writefds, fd_set *exceptfds, struct timeval *timeout)
{
    int error;
    if ((error = select(nfds, readfds, writefds, exceptfds, timeout)) < 0) {
        errx(1, "select:");
    }
    return error;
}

int pollw(struct pollfd *fds, unsigned long nfds, int timeout)
{
    int error;
    if ((error = poll(fds, nfds, timeout)) < 0) {
        errx(1, "poll:");
    }
    return error;
}